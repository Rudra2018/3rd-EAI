...
Usage (compat with older code):
    from utils.vrt import VRTMapper
    mapper = VRTMapper()  # or VRTMapper("path/to/bugcrowd_vrt.json")
    decision = mapper.classify("Auth Bypass / Broken Authentication", ctx)
    print(decision.priority, decision.severity, decision.rationale)

If you place the official VRT JSON beside this file (e.g., ./bugcrowd_vrt.json),
we’ll try to load it automatically; otherwise we use the internal defaults.
...
__all__ = ["VRTDecision", "VRT", "VRTMapper"]
